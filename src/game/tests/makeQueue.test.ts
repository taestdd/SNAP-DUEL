...
// attack_weak_punch 포즈: frames 3개, fps 5(모든 포즈 공통) → 프레임당 200ms (스프라이트 'a' 기준)
...
describe("frame → ms 환산", () => {
  it("frame 1은 5fps 포즈에서 200ms에 발화한다", () => {
...
describe("히트스탑 인지 다단히트", () => {
  it("두 번째 히트는 첫 히트의 freeze만큼 뒤로 밀린다", () => {
    ...
    // hit1: 200ms(frame) + 0(acc)
    expect(hits[0].delay).toBe(200);
    // hit2: 200ms(frame) + 100(앞선 freeze)
    expect(hits[1].delay).toBe(300);
...
describe("대시-인 (근접공격)", () => {
  it("비근접 + 근접공격 + 적중: 상대에 겹치는 위치까지 대시 후 시퀀스가 DASH_MS만큼 밀린다", () => {
    ...
    expect(visualHits(events)[0].delay).toBe(DASH_MS + 200);
  });
  ...
  it("빗나간 근접공격은 휘핑 — 돌진 후 헛스윙하고 원위치로 복귀한다", () => {
    ...
    // 임팩트 프레임(200ms) 직후 복귀 — bgPush 없음 (아무것도 맞지 않았으므로)
    expect(mv[1]).toMatchObject({
      subject: "P1",
      toOffset: HOME_OFFSET.P1,
      motion: "recover",
      delay: DASH_MS + 200 + WHIFF_RETURN_MS,
    });
